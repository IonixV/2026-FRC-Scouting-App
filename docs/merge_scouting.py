...
EXPECTED_COLS = [
    ...
    "Balls/Sec", "Accuracy %", "Field Zone", "Endgame Climb",
...
def main():
    ...
    all_widths = [8,9,9,14,10,14,12,10,12,12,14,14,13,14,35,22,18]
    ...
    team_widths = [9,8,9,10,14,12,10,12,12,14,14,13,14,35]
    df_team = df.sort_values(["Team","Match"])
    write_sheet(wb, "By Team", df_team, team_cols, team_widths, "1F8E3C")

    # ── Sheet 3: Summary ──────────────────────────────────────────────────
    numeric_df = df.copy()
    for col in ["Auto Fuel","Balls/Sec","Accuracy %","Defense Rating","Driver Rating"]:
    ...
    summary["Avg_Balls_Sec"] = summary["Avg_Balls_Sec"].round(1)
    summary["Avg_Accuracy"]  = summary["Avg_Accuracy"].round(1)
    ...
    summary.sort_values("Avg_Balls_Sec", ascending=False, inplace=True)

    summary.rename(columns={
        "Avg_Auto_Fuel": "Avg Auto Fuel",
        "Avg_Balls_Sec": "Avg Balls/Sec",
    ...
    sum_cols   = ["Team","Matches","Avg Auto Fuel","Avg Balls/Sec","Avg Accuracy %",
                  "Avg Endgame Pts","Avg Defense","Avg Driver Skill"]
    sum_widths = [9,9,14,14,15,15,13,16]
